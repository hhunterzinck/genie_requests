# Description: Check why date is misformatted vicc panc upload.
...
col_date <- grep(pattern = "date", x = colnames(vicc), value = T)
head(vicc[,col_date])
head(uhn[,col_date])

col_time <- grep(pattern = "time", x = colnames(vicc), value = T)
head(vicc[,col_time])
head(uhn[,col_time])
