      ...
      if (is.element(cohort_child$name, sites)) {
        ...
        for(site_child in site_children) {
          if (grepl(pattern = glue("{site} {cohort} Data"), x = site_child$name)) {
            syn_ann <- synSetAnnotations(site_child$id, annotations=list(cohort = cohort, site = site, file_type = "data", phase = "2"))
          } else if (grepl(pattern = glue("{site} {cohort} Header"), x = site_child$name)) {
            syn_ann <- synSetAnnotations(site_child$id, annotations=list(cohort = cohort, site = site, file_type = "header", phase = "2"))
          }
        }
      }
    }
