# Description: Check formats of dates in the seq-date column.
...
sam <- get_synapse_entity_data_in_csv(synid_file_sam, version = synid_ver_sam, sep = "\t")

# main ----------------------------

sam %>% 
  mutate(SEQ_MONTH = unlist(lapply(strsplit(SEQ_DATE, split = "-"), function(x) {return(x[1])}))) %>%
  select(SEQ_MONTH) %>%
  group_by(SEQ_MONTH) %>%
  count() %>%
  arrange(SEQ_MONTH)
...
sam %>% 
  mutate(SEQ_MONTH = unlist(lapply(strsplit(SEQ_DATE, split = "-"), function(x) {return(x[1])}))) %>%
  filter(!is.element(SEQ_MONTH, allowed))
